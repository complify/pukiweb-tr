    ...
    <footer className="border-t border-[#e7ebf1] bg-white">
      <div className="container-p py-10 grid gap-8 md:grid-cols-4 text-sm">
        <div>
          ...
          <p className="text-muted leading-relaxed">{d.tagline}</p>
        </div>
        <div>
          <div className="font-bold text-ink mb-3">{d.product}</div>
          <ul className="space-y-2 text-[#5e6278]">
            ...
            <li><Link href="/demo" className="hover:text-puki-dark">{d.demoLink}</Link></li>
            <li><Link href="/hesap" className="hover:text-puki-dark">{d.account}</Link></li>
          </ul>
        </div>
        <div>
          <div className="font-bold text-ink mb-3">{d.standards}</div>
          <ul className="space-y-2 text-[#5e6278]">
      ...
      <div className="border-t border-[#eef1f6]">
        <div className="container-p py-5 flex flex-col md:flex-row items-center justify-between gap-4">
          <span className="text-xs text-muted order-2 md:order-1">© {new Date().getFullYear()} Puki Yazılım Teknoloji A.Ş. {d.rights}</span>
          <div className="flex items-center gap-3 order-1 md:order-2">
            <span className="text-[.7rem] font-semibold text-muted">{d.securePayment}</span>
            <PaymentMarks />
          </div>
        </div>
      </div>
