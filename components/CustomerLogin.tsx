...
export default function CustomerLogin() {
  const p = useDict().portal;
  const sp = useSearchParams();
  const initialErr = sp.get("error") ? p.loginErr : null;
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErr(null);
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email.trim())) { setErr(p.emailInvalid); return; }
    setBusy(true);
    try {
      const r = await fetch("/api/hesap/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email }) });
      if (!r.ok) { const d = await r.json(); throw new Error(d.error || p.linkFailed); }
      setSent(true);
  ...
  if (sent) {
    return (
      <div className="bg-white border border-[#e7ebf1] rounded-xl3 shadow-card p-8 text-center">
        ...
        <h2 className="mt-5 text-xl font-extrabold text-ink">{p.sentTitle}</h2>
        <p className="text-[#5e6278] mt-2 text-sm leading-relaxed"><b>{email}</b>{p.sentMsg}</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={submit} className="bg-white border border-[#e7ebf1] rounded-xl3 shadow-card p-8">
      <label className="block">
        <span className="text-sm font-semibold text-ink">{p.emailLabel}</span>
      ...
      <p className="text-xs text-muted mt-3 text-center">{p.noPassword}</p>
    </form>
  );
}
