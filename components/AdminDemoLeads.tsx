...
const fmtDate = (ts: number) => new Date(ts).toLocaleString("tr-TR", { dateStyle: "medium", timeStyle: "short" });
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div className="text-sm text-muted">
          {leads.length} talep · Depo: <b className={kvEnabled ? "text-puki-dark" : "text-amber-600"}>{kvEnabled ? "Kalıcı (KV)" : "Bellek içi (geçici)"}</b>
        </div>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="text-sm text-[#5e6278] mt-1.5">
                    <span className="font-semibold text-ink">{l.company}</span> · {l.name} ·{" "}
                    <a href={`mailto:${l.email}`} className="text-puki-dark">{l.email}</a>
                    {l.phone ? <> · <a href={`tel:${l.phone}`} className="text-puki-dark">{l.phone}</a></> : null}
                  </div>
                  {l.modules.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {l.modules.map((c) => (
                        <span key={c} className="text-[.7rem] font-bold text-puki-dark bg-puki-light px-2 py-0.5 rounded-full">{modName(c)}</span>
                      ))}
                    </div>
                  )}
